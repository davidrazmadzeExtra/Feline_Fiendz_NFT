...
contract FelineFiendz is Delegated, FF721Batch, Merkle, PaymentSplitterMod {
    ...
    uint256[] private _shares = [1966, 1966, 1966, 1966, 983, 983, 17];
    ...
    function mint(uint256 quantity, bytes32[] calldata proof) external payable {
        require(
            0 < quantity && quantity <= MAX_ORDER,
            "FelineFiendz: order too big"
        );
        require(
            msg.value >= PRICE * quantity,
            "FelineFiendz: ether sent is not correct"
        );
        require(
            owners[msg.sender].purchased + quantity <= MAX_MINT,
            "FelineFiendz: don't be greedy"
        );
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "FelineFiendz: mint/order exceeds supply"
        );
        ...
        onlyDelegates
    {
        require(
            ...
            "FelineFiendz: must provide equal quantities and recipients"
        );
    ...
    function resurrect(
        ...
        require(
            tokenIds.length == recipient.length,
            "FelineFiendz: must provide equal tokenIds and recipients"
        );
    ...
    function setActive(bool isPresaleActive_, bool isMainsaleActive_)
        external
        onlyDelegates
    {
    ...
    function setBaseURI(string calldata _newPrefix, string calldata _newSuffix)
        external
        onlyDelegates
    {
    ...
    function setConfig(
        ...
        require(
            ...
            "FelineFiendz: specified supply is lower than current balance"
        );
        MAX_ORDER = maxOrder;
    ...
    function _resurrect(address to, uint256 tokenId) private {
        require(
            !_exists(tokenId),
            "FelineFiendz: can't resurrect existing token"
        );
        --burned;
